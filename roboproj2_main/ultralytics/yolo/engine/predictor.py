...
class BasePredictor:
    ...
    def setup_source(self, source=None):
        ...
        if webcam:
            ...
            self.dataset = LoadStreams(source,
                                       ...
                                       auto=pt,
                                       transforms=getattr(
                                           self.model.model, 'transforms', None),
    ...
    def __call__(self, source=None, model=None, verbose=False, stream=False):
        if stream:
            ...
            return list(self.stream_inference(source, model, verbose))
    ...
    def stream_inference(self, source=None, model=None, verbose=False):
        ...
        if not self.done_warmup:
            self.model.warmup(
                imgsz=(1 if self.model.pt or self.model.triton else self.bs, 3, *self.imgsz))
        ...
        for batch in self.dataset:
            ...
            for i in range(len(im)):
                p, im0 = (path[i], im0s[i]) if self.webcam or self.from_img else (
                    path, im0s)
                p = Path(p)
    ...
    def setup_model(self, model):
        ...
        self.model = AutoBackend(model, device=device,
